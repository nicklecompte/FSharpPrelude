...
module NumericTypes =
    ...
    type Ratio(valu:float) = // todo: Reference type should be OK here since it won't get called very often.
        member x.Value =
            match valu < 1.0 with
            | true -> valu
            | false ->
                match valu < 100.05 with
                | true -> valu / 100.0
                | false -> invalidArg "valu" $"Ratio must be given a value less than 100.05, was given %f{valu}"
    
    [<AutoOpen>]
    /// Contains specific complex types for the .NET built-in numerics, along
    ...
    module Complex =
        ...
        /// TODO underlying unmanaged numeric type.
        [<Struct>]
        type ValueComplex< 'T when 'T : equality and 'T : struct> = {
                re : 'T
                im : 'T
        }
